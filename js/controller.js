(function () {
  nocModule.controller(
    'SingleCellAutomataCtrl',
    function ($scope, $timeout, utils, colors, fullscreenService, defaults) {
      $scope.rules = {
        '000': false,
        '001': true,
        '010': false,
      ...
      $scope.ruleKeys = _.keys($scope.rules).sort().reverse();
      $scope.speed = 5;
      $scope.scale = 5;
      $scope.stateY = 0;
      ...
      $scope.toggleRuleByIdx = function (idx) {
        if (idx < $scope.ruleKeys.length) {
          $scope.rules[$scope.ruleKeys[idx]] =
            !$scope.rules[$scope.ruleKeys[idx]];
        }
      };
      ...
      $scope.downloadImage = function () {
        ...
        utils.downloadCanvasImageFromLink(link, canvas, 'cell_automata.png');
      };
